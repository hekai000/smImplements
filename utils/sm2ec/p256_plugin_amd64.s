...
// 1.p256Sqr
// 2.p256OrdSqr
// 3.mysm2P256MulInternal
// 4.mysm2P256SqrInternal
// The most affected one is mysm2P256MulInternal, it uses SIMD register X0 as temp storage.
...
TEXT mysm2P256MulInternal(SB),NOSPLIT,$8
	CMPB ·supportBMI2+0(SB), $0x01
	JEQ  internalMulBMI2
...
TEXT mysm2P256SqrInternal(SB),NOSPLIT,$8
	CMPB ·supportBMI2+0(SB), $0x01
	JEQ  internalSqrBMI2
...
#define p256PointAddAffineInline() \
	...
	\// Begin point add
	LDacc (z1in)                      \
	CALL mysm2P256SqrInternal(SB)	      \// z1ˆ2
	ST (z1sqr)                        \
	\
	LDt (x2in)                        \
	CALL mysm2P256MulInternal(SB)	      \// u2 = x2 * z1ˆ2
	\
	LDt (x1in)                        \
	...
	\
	LDt (z1in)                        \
	CALL mysm2P256MulInternal(SB)	      \// z3 = h * z1
	ST (zout)                         \
	\
	LDacc (z1sqr)                     \
	CALL mysm2P256MulInternal(SB)	      \// z1ˆ3
	\
	LDt (y2in)                        \
	CALL mysm2P256MulInternal(SB)	      \// s2 = y2 * z1ˆ3
	ST (s2)                           \
	\
	LDt (y1in)                        \
	...
	ST (r)                            \
	\
	CALL mysm2P256SqrInternal(SB)	      \// rsqr = rˆ2
	ST (rsqr)                         \
	\
	LDacc (h)                         \
	CALL mysm2P256SqrInternal(SB)	      \// hsqr = hˆ2
	ST (hsqr)                         \
	\
	LDt (h)                           \
	CALL mysm2P256MulInternal(SB)	      \// hcub = hˆ3
	ST (hcub)                         \
	\
	LDt (y1in)                        \
	CALL mysm2P256MulInternal(SB)	      \// s2 = y1 * hˆ3
	ST (s2)                           \
	\
	LDacc (x1in)                      \
	LDt (hsqr)                        \
	CALL mysm2P256MulInternal(SB)	      \// x1 * hˆ2
	ST (h)                            \
	\
...
#define p256PointAddInline() \
	\// Begin point add
	LDacc (z2in)                 \
	CALL mysm2P256SqrInternal(SB)	 \// z2ˆ2
	ST (z2sqr)                   \
	LDt (z2in)                   \
	CALL mysm2P256MulInternal(SB)	 \// z2ˆ3
	LDt (y1in)                   \
	CALL mysm2P256MulInternal(SB)	 \// s1 = z2ˆ3*y1
	ST (s1)                      \
	\
	LDacc (z1in)                 \ 
	CALL mysm2P256SqrInternal(SB)	 \// z1ˆ2
	...
	CALL mysm2P256MulInternal(SB)	 \// z1ˆ3
	LDt (y2in)                   \
	CALL mysm2P256MulInternal(SB)	 \// s2 = z1ˆ3*y2
	ST (s2)                      \ 
	\
	LDt (s1)                     \
	...
	LDacc (z2sqr)                \
	LDt (x1in)                   \
	CALL mysm2P256MulInternal(SB)	 \// u1 = x1 * z2ˆ2
	ST (u1)                      \
	LDacc (z1sqr)                \
	LDt (x2in)                   \ 
	CALL mysm2P256MulInternal(SB)	 \// u2 = x2 * z1ˆ2
	...
	LDacc (z1in)                 \
	LDt (z2in)                   \
	CALL mysm2P256MulInternal(SB)	 \// z1 * z2
	LDt (h)                      \
	CALL mysm2P256MulInternal(SB)	 \// z1 * z2 * h
	ST (zout)                    \
	\
	LDacc (hsqr)                 \
	LDt (u1)                     \
	CALL mysm2P256MulInternal(SB)	 \// hˆ2 * u1
	ST (u2)                      \
	\
...
#define calZ() \
	...
	LDacc (z)                               \
	LDt (y)                                 \
	CALL mysm2P256MulInternal(SB)             \ // Z1 * Y1
...
#define calX() \
	...
	LDt (m)                                 \
	CALL mysm2P256MulInternal(SB)             \ // M = (X1 - ZZ) * (X1 + ZZ) = X1^2 - ZZ^2
	ST (m)                                  \
	\// Multiply by 3
	p256TripleInline                        \
	STt (m)                                 \  // M = 3 * (X1^2 - ZZ^2)
	...
	LDacc (y)                               \
	p256MulBy2Inline2                       \
	CALL mysm2P256SqrInternal(SB)             \ // 4 * YY = (2*Y1)^2
	ST (s)                                  \ // S = 4 * YY
	CALL mysm2P256SqrInternal(SB)             \ // (4 * YY)^2 = 16 * YYYY
	\// Divide by 2
	XORQ mul0, mul0                         \
	MOVQ acc4, t0                           \
	MOVQ acc5, t1                           \  
	...
	LDacc (x)                               \
	LDt (s)                                 \
	CALL mysm2P256MulInternal(SB)             \ // X1 * 4 * YY
	ST (s)                                  \ // S = 4 * X1 * YY = 2 * ((X1+YY)^2 - XX - YYYY)
	p256MulBy2Inline                        \
	STt (tmp)                               \ // tmp = 2*S = 8 * X1 * YY
	\
	LDacc (m)                               \
	CALL mysm2P256SqrInternal(SB)             \ // M^2 = (3 * (X1^2 - ZZ^2))^2
	LDt (tmp)                               \
...
#define calY() \
	...
	\
	LDt (m)                                 \
	CALL mysm2P256MulInternal(SB)             \ // M * (S - X3)
	\
	LDt (y)                                 \
