...
TEXT mysm2P256SqrInternal<>(SB),NOSPLIT,$0
	// x[1:] * x[0]
	MUL	x0, x1, acc1
	UMULH	x0, x1, acc2
...
TEXT mysm2P256MulInternal<>(SB),NOSPLIT,$0
	// y[0] * x
	MUL	y0, x0, acc0
	UMULH	y0, x0, acc1
...
TEXT ·p256PointAddAffineAsm(SB),0,$264-48
	...
	LDx(x2in)
	CALL	mysm2P256MulInternal<>(SB)    // x2 * z1ˆ2
	...
	LDy(z1sqr)
	CALL	mysm2P256MulInternal<>(SB)    // z1 ^ 3
	...
	LDx(y1in)
	CALL	mysm2P256MulInternal<>(SB)    // y1 * hˆ3
	STy(s2)
	...
	CALL	mysm2P256MulInternal<>(SB)    // u1 * hˆ2
	STP	(y0, y1), h(0*8)
	STP	(y2, y3), h(2*8)
...
TEXT ·p256PointAddAsm(SB),0,$392-32
	...
	// Begin point add
	LDx(z2in)
	CALL	mysm2P256SqrInternal<>(SB)    // z2^2
	STy(z2sqr)

	CALL	mysm2P256MulInternal<>(SB)    // z2^3
	...
	LDx(z1in)
	CALL	mysm2P256SqrInternal<>(SB)    // z1^2
	STy(z1sqr)
